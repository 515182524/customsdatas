...
def normalize_report_spacing(blocks):
    normalized = []
    ...
    for block in blocks:
        if block == "":
            ...
            previous_was_table = False
            continue
        is_image = block.startswith("![")
        is_table = block.startswith("| ")
        starts_table = is_table and not previous_was_table
        ends_table = previous_was_table and not is_table
        if (is_image or starts_table or ends_table) and normalized and normalized[-1] != "":
            normalized.append("")
...
def main():
    ...
    report_path.write_text("\n".join(normalize_report_spacing(report)) + "\n", encoding="utf-8")
    print(report_path)
